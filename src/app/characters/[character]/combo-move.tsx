...
function ComboMove({ type, input, index, onRemove, version }: { type: MoveType, input: string, version?: MoveStrength; index: number, onRemove: (index: number) => void }) {
    return <div className={style["combo-move"]} style={{ position: "relative" }}>{type === "special" && version ? getInputWithStrength(input, version) : type === "drive-rush" ? "Drive Rush" : input}
    <button onClick={() => {
        onRemove(index);
